...
from ELGamal import encode_file,uncode_file
...
class client_no_ssl:
    ...
    def download(self, filename):
        ...
        if buf:  # 如果不加这个if，第一个文件传输完成后会自动走到下一句
            ...
            if stat == 'Success':
                fileSize = header['fileSize']
                filenewname = os.path.join('ELGamal/serverfile/', filename)
    ...
    def register(self,username,password):
        ...
        buf = self.ssock.recv(fileinfo_size)
        if buf:  # 如果不加这个if，第一个文件传输完成后会自动走到下一句
            header_json = str(struct.unpack('128s', buf)[0], encoding='utf-8').strip('\x00')
            print(header_json)
            header = json.loads(header_json)
            stat = header['stat']
            if stat == 'Success':
...
if __name__ == "__main__":
    ...
    filepath = 'cer/server/server.crt'
    client.login('sanfen','sanfen')
